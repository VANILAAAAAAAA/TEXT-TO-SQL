...
async function fetchDatabases() {
    try {
        const response = await fetch('http://172.23.4.44:8080/get_databases', {
            method: 'POST'
        });
...
async function getTables() {
    ...
    try {
        const response = await fetch('http://172.23.4.44:8080/get_tables', {
            method: 'POST',
        ...
        if (data.tables.length > 0) {
            selectedTable = data.tables[0]; // 选择第一个表
            tableSelectDropdown.value = selectedTable; // 设置下拉菜单的值
...
async function getDatabaseDetails() {
    ...
    try {
        const response = await fetch('http://172.23.4.44:8080/get_database_details', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function fetchUserInputs() {
    ...
    try {
        const response = await fetch('http://172.23.4.44:8080/get_user_inputs', {
            method: 'GET'
        });
...
document.getElementById('askModelButton').onclick = async function() {
    ...
    try {
        const response = await fetch('http://172.23.4.44:8080/get_model_output', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function executeSQLQuery(sqlQuery) {
    ...
    try {
        const response = await fetch('http://172.23.4.44:8080/execute_query', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                database: database,
...
document.getElementById('userQuestion').addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
        if (event.shiftKey) {
            // 如果按下了 Shift + Enter，允许换行
            return; // 不做任何操作
        } else {
            // 如果只按下 Enter，发送消息
            event.preventDefault(); // 防止默认的换行行为
            document.getElementById('askModelButton').click(); // 模拟点击发送按钮
