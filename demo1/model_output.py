...
async def init_db(database: str):
    ...
    if current_database != database:
        ...
        await Tortoise.init(
            db_url=f"mysql://root:12345678@localhost:3306/{database}",
...
async def get_model_output_route(request: Request):
    ...
    model_output = await get_model_output_from_api(prompt, question)
    user_inputs.append({"prompt": prompt, "question": question, "model_output": model_output})
...
@app.get('/get_user_inputs')
async def get_user_inputs_route():
    """获取最近的用户输入"""
    return JSONResponse(content={"user_inputs": user_inputs[-10:]})  # 返回最近的10条用户输入
